...
export default function Login() {
  ...
  const handleGoogleCredential = async (response: any) => {
    ...
    try {
      ...
      setMessage(`Error: ${error.message || "Google authentication failed"}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleFallbackClick = () => {
    alert(
      "Google Sign-In Client ID is not configured. Please add NEXT_PUBLIC_GOOGLE_CLIENT_ID to your .env.local file to activate Google login."
    );
  };
